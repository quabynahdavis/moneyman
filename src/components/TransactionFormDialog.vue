...
const totalDebits = computed(() =>
  splits.value.reduce((s, r) => s + (parseFloat(r.debitAmount) || 0), 0),
)
const totalCredits = computed(() =>
  splits.value.reduce((s, r) => s + (parseFloat(r.creditAmount) || 0), 0),
)
...
function autoBalance() {
  ...
  // Find the last empty row or the last row
  const last = splits.value[splits.value.length - 1]
  if (diff > 0) {
    last.creditAmount = diff.toFixed(2)
  } else {
    last.debitAmount = (-diff).toFixed(2)
  }
}

async function save() {
  if (!isBalanced.value) {
    error.value = "Transaction must be balanced (debits = credits)"
    return
  }
  ...
  try {
    const txn = await txnStore.postNewTransaction({
      date: date.value,
...
<template>
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-2 gap-3">
      ...
      <div class="space-y-1">
        <Label>Number</Label>
        <Input v-model="number" placeholder="Check #" />
      </div>
      <div class="space-y-1 col-span-2">
        <Label>Payee</Label>
        <Input v-model="payee" placeholder="Payee or counterparty" />
      </div>
      <div class="space-y-1 col-span-2">
        <Label>Description</Label>
        <Input v-model="description" placeholder="Transaction memo" />
      </div>
    </div>
    ...
    <div>
      <div class="flex items-center justify-between mb-2">
        <Label>Splits</Label>
        <Button variant="outline" size="sm" @click="addSplit">
          <Plus class="h-3 w-3 mr-1" /> Add Split
        </Button>
      </div>
          ...
          <tbody class="divide-y">
            <tr v-for="(split, idx) in splits" :key="split.id">
              <td class="px-3 py-1.5">
                <select
                  v-model="split.accountId"
                  class="w-full rounded border border-input bg-background px-2 py-1 text-xs ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                >
                  <option :value="null">Select account...</option>
                  <option
                    v-for="acc in leafAccounts"
                    :key="acc.id"
                    :value="acc.id"
                  >
                    {{ acc.name }}
              ...
              <td class="px-3 py-1.5">
                <input
                  v-model="split.memo"
                  placeholder="Memo"
                  class="w-full rounded border border-input bg-background px-2 py-1 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                />
              </td>
          ...
          <tfoot class="bg-muted/30 border-t">
            <tr>
              <td class="px-3 py-1.5 text-xs font-medium text-muted-foreground">
                Totals
                <span v-if="!isBalanced" class="ml-2 text-destructive font-semibold">
                  (Off by {{ formatBalance(Math.abs(totalDebits - totalCredits)) }})
                </span>
                <span v-else class="ml-2 text-emerald-500 font-semibold">Balanced ✓</span>
              </td>
              <td class="px-3 py-1.5 text-right text-xs font-mono">{{ formatBalance(totalDebits) }}</td>
              <td class="px-3 py-1.5 text-right text-xs font-mono">{{ formatBalance(totalCredits) }}</td>
              <td></td>
              <td></td>
            </tr>
          </tfoot>
      ...
      <div class="flex justify-end mt-2">
        <Button variant="ghost" size="sm" @click="autoBalance" :disabled="isBalanced">
          Auto-Balance
        </Button>
      </div>
    </div>
    ...
    <div class="flex justify-end gap-2 pt-2 border-t">
      ...
      <Button :disabled="saving || !isBalanced" @click="save">
